...
- `.github/instructions/prompt-architecture.instructions.md`：项目元架构规范，约束代码、文档和 prompt/skill/reference/workflow 的边界性、正交可组合性、简约性、闭环和自我迭代。
...
Skills 只负责少量外部维度和入口输出协议：`architecture-planner` 使用 `mode` 与 `target`，`black-team-review` 使用 `subject` 与 `mode`。组合方式由 `.github/instructions/ousia-workflow.instructions.md`、`.github/instructions/prompt-architecture.instructions.md` 和入口 skill 的“组合资产”段约束；这些 prompt 结构也必须服从项目元架构规则。
