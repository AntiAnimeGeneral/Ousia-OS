...
Skills 只负责少量外部维度和入口输出协议：`architecture-planner` 使用 `mode` 与 `target`，`black-team-review` 使用 `subject` 与 `mode`。组合方式由 `.github/instructions/ousia-workflow.instructions.md` 和入口 skill 的“组合资产”段约束。
