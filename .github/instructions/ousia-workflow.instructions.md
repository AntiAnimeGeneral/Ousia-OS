...
- 调用 review subagent 时，必须显式指定与当前主上下文同型号、且带 provider 后缀的完整模型名，例如 `gpt-5.5::fxh (oaicopilot)`。不要使用裸型号名、`Auto`、Copilot 默认模型或任何隐式 fallback。
- 如果指定失败或当前工具无法显式指定同型号模型，跳过该 subagent review，并在最终报告中说明原因，把未运行同型号 review 标记为剩余风险。
- subagent prompt 必须要求 subagent 读取对应 skill，并说明当前 review 类型、用户目标、输入材料、范围、已运行或计划运行的检查、review focus 和输出格式。
- 架构提案进入实施前，使用 [architecture-proposal-review skill](../skills/architecture-proposal-review/SKILL.md) 审查 proposal packet。
- 完成非平凡实现、重构、架构边界调整或行为变更后，使用 [implementation-diff-review skill](../skills/implementation-diff-review/SKILL.md) 审查真实 diff、测试结果和行为风险。
