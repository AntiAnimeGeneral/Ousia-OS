...
- Kernel 语义改动按测试层级选择检查：单一 owner 的权限、类型、状态转换或错误映射改动运行对应 unit tests；跨 CSpace/ObjectTable/ThreadTable/Scheduler 的 invocation 或事务行为改动运行 host integration tests；`kernel-bin`、`ostd`、linker、QEMU runner 或 boot marker 改动才要求 QEMU smoke。
