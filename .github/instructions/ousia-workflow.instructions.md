...
- 涉及 `kernel/**`、`design/implementation/**` 或 seL4 baseline 语义的非平凡实现，不从 Ousia 当前代码局部补洞开始；先读取对应本地 seL4/rust-sel4 reference 和 owning docs，抽取 baseline 语义，再决定 Rust API、owner 边界、错误边界和测试形状。
- 实施前至少能说明：对应 seL4 源码路径、decode/lookup/validation 顺序、authority root、状态 owner、副作用提交点、失败无副作用不变量，以及 Ousia Rust 表达与 seL4 C 实现的映射关系。说不清时先补 reference 阅读或交给 architecture planner，不要继续编码。
- 实施中发现当前 Ousia helper、fixture、descriptor facade 或容器形状与 baseline 冲突时，以 baseline 目标为准修正边界；不要为了低风险或少改文件保留错误兼容入口。
- 测试先写 seL4 baseline 行为契约：谁调用、source/target/destination authority 从哪来、错误顺序是什么、失败后哪些 owner state 不变。测试不得只是把旧 descriptor 包进新 path 或复述 helper 返回值。
- 最终报告和 review inputs 必须列出本轮读取或核对的本地 reference 路径；如果未读取、未能映射或只做了局部猜测，把 baseline drift 标为 residual risk。
