...
- 非平凡测试应被写成轻量行为契约：用领域词汇说明 Arrange、Act、Assert，明确前置状态、动作、可观察结果和失败后保持不变的 owner 状态。不要把内部 helper 调用顺序当作测试语义。
- 优先写端到端边界、状态转移、不变量、权限/能力语义、失败无副作用和跨模块协作测试。
- 纵向切片的测试必须证明声明的语义路径穿过真实 owner 和边界后成立，例如从 syscall/descriptor 到 owner state、commit、副作用队列和可观察结果；不要只测试 helper、重命名后的结构或局部 match 表后宣称切片完成。
...
- `Goal`、`Scope` 和 `Semantics` 是 Rust 测试内的轻量行为契约。它们应描述调用方、状态 owner、硬件/平台边界或系统链路观察到的行为，不应描述 helper 调用顺序、局部变量安排或实现里的 match 分支。
