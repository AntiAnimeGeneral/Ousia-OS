...
- Object Store、Stream 和 MemoryObject 共同构成原生数据与 VM 模型；FS 放置在纯用户态与纯内核态两个候选间裁决。
...
| MemoryObject / Pager        | 可分页内存对象；在纯用户态 FS 方案中由用户态 Pager 供页，在纯内核态 FS 方案中可由内核 Object Store 直接供页 | [03-pager-and-memory.md](./core/03-pager-and-memory.md)             |
...
| 第 1 层：基础系统服务   | 名字服务、Capsule 管理器、网络、设备管理、Driver Manager/Index/Host、日志与观测；纯用户态 FS 方案还包含对象存储与 Pager 监督 |
...
4. MemoryObject、缺页处理，以及纯用户态 Pager / 纯内核 Object Store 两条供页路径。
5. 最小 Object Store 与目录树兼容投影；裁决其作为用户态服务还是内核原语落地。
