...
本文档是 Ousia OS 设计文档的入口。它只回答四件事：现有系统哪里错了，Ousia OS 必须满足哪些硬需求，系统要达成什么目标，以及这些需求和目标分别由哪些主线章节承接。
...
本文作为设计入口和主线索引，约束各专题文档的需求边界、目标边界与阅读顺序。
...
2. 再读本文的硬需求、目标和判断标准，确认系统必须满足什么、要解决什么、不解决什么。
...
抽象设计必须先满足硬需求，再命名原语和划分边界。硬需求不是实现细节偏好，而是第一阶段验收条件；如果某个抽象不能自然满足这些条件，就说明抽象不对。

| 需求                                          | 验收条件                                                                                     | 直接推出的抽象                                                        |
| --------------------------------------------- | -------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| 类 FUSE 的存储接入                            | 本地、远程、加密、同步和兼容投影存储都能作为 provider 接入系统                               | FS Provider、ProviderRoot、MountBinding、ObjectHandle                 |
| FS 可挂载到目录                               | native provider 的目录下可挂载 remote provider；路径解析跨 provider 后仍返回统一 ObjectHandle | Object Namespace / VFS-like 层、NameBinding、NamespaceView            |
| mmap 必须是原生能力                           | 文件/对象可映射为 MemoryObject；缺页路径有明确供页者、故障模型、优先级和回写边界             | MemoryObject、Pager、page fault queue、Lease / Version / invalidate   |
| 大数据路径支持 zero-copy / low-copy           | 大块 IO、mmap、设备 DMA、共享缓冲区和 provider fast path 不因抽象边界被迫复制                 | IOBuffer、MemoryDescriptor、SharedQueue、TransferArena、IOMMU 授权    |
| 同步与异步都是一等调用形态                    | 小控制面支持低延迟 sync call；长请求可取消、超时、等待、late reply                            | Portal、Operation、Continuation、SaveHandle、EventPort / WaitSet      |
| 用户态服务不能牺牲热路径                      | 高频 FS/driver/network 路径具备 fast call、batch、queue、bypass 或 direct descriptor 形态      | Portal fast call、SharedQueue、KernelChannel、bypass session          |
| 权限必须可组合、可审计、可撤销                | 打开、挂载、mmap、DMA、跨 Capsule 传递都必须由 Capability 表达并可撤销                        | Capability、ObjectHandle、DMA mapping capability、generation invalidation |
| 兼容性不能污染原生 API                        | POSIX open/read/write/mount/fuse 由兼容域或网关翻译，不成为原生对象模型                       | Compatibility Domain、POSIX projection、Object API                   |
| 远程资源必须是一等场景                        | 远程 FS、远程服务和远程对象的延迟、断连、一致性和 durability fence 有系统级表达               | FS Provider、Remote-backed MemoryObject、Lease、EventPort、Fence      |
| 安装、升级、回滚和多版本并存必须系统化        | 应用不依赖 PATH/bashrc/profile 拼装；依赖和生命周期由系统记录、激活和回滚                    | Package Cell、Service Graph、Environment / Config Service             |

这些需求的作用是压缩设计空间。例如“类 FUSE 接入 + 可挂载到目录 + mmap + zero-copy”同时成立时，系统不能只提供私有 RPC 文件服务，也不能只提供 POSIX VFS；它必须有 Object Namespace、FS Provider、MemoryObject 和受控共享内存/队列。类似地，“同步与异步一等 + 用户态服务热路径”同时成立时，Communication Fabric 不能只是 Future 框架，也不能只是阻塞 IPC。
...
- Object Namespace、Object Store、Stream 和 MemoryObject 共同构成原生命名、数据与 VM 模型。
