...
本文档树是持续演进的草案，不是冻结后的实现说明。实现结果只应作为证据和反馈来源，不应反过来锁死更好的抽象、边界或阶段顺序；如果更好的方案出现，应优先重构文档与实现边界。
...
- 权限与兼容：Capability 授权、审计、内核硬撤销、语义失效，以及 POSIX 兼容域边界。
...
| Capability                  | 不可伪造的权限句柄，绑定对象、操作和内核可见派生关系                        | [01-capsule-and-capability.md](./core/01-capsule-and-capability.md) |
...
- 不开放不受控内核扩展接口。
- 不承诺任意用户态语义委托的全局回滚；内核可见能力仍必须支持派生链硬撤销。
...
1. 是否消除了隐式全局状态？
2. 内核可见权限是否显式、可审计、可硬撤销，服务语义授权是否可失效？
