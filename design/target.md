...
主线章节：[01-package-cell.md](./core/01-package-cell.md)，边界专题：[04-environment-and-config.md](./topics/04-environment-and-config.md)。

### 1.2 默认权限模型过宽

传统进程默认继承用户身份、全局文件系统视图、网络能力和大量环境状态。沙盒通常是额外补丁，而不是系统的默认运行方式。

主线章节：[02-capsule-and-capability.md](./core/02-capsule-and-capability.md)。

### 1.3 同步阻塞与粗糙调度不适合现代交互系统

现代工作负载高度并发，CPU、GPU、NPU、IO 队列和内存带宽都会成为竞争资源。现有系统很难统一表达取消、超时、背压、前台保活、实时约束和跨设备资源预算。

主线章节：[06-compute-and-scheduling.md](./core/06-compute-and-scheduling.md)，通信基础见 [08-communication-fabric.md](./core/08-communication-fabric.md)。

### 1.4 文件系统抽象落后于现代数据使用方式

“目录树 + 字节流文件”是有用的兼容视图，但不应是原生数据模型的唯一真相。现代应用需要稳定对象 ID、元数据、索引、版本、事务、流和配置语义。

主线章节：[04-data-and-filesystem.md](./core/04-data-and-filesystem.md)，VM 细节见 [05-pager-and-memory.md](./core/05-pager-and-memory.md)。
...
主线章节：[06-compute-and-scheduling.md](./core/06-compute-and-scheduling.md)，驱动与硬件边界见 [07-driver-and-kernel.md](./core/07-driver-and-kernel.md)。

### 1.7 抽象边界不能牺牲关键路径性能

用户态服务、微内核、能力模型和异步接口都不能成为性能借口。高频控制面、异步请求、大数据路径和设备 fast path 必须各自走最低成本路径。

主线章节：[08-communication-fabric.md](./core/08-communication-fabric.md)，旁路和驱动路径见 [07-driver-and-kernel.md](./core/07-driver-and-kernel.md)。
...
| 抽象                        | 作用                                                                        | 主线章节                                                            |
| --------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Package Cell                | 软件交付、依赖、环境和生命周期单元                                          | [01-package-cell.md](./core/01-package-cell.md)                     |
| Capsule                     | 运行隔离域，包含地址空间、线程、能力集合和资源预算                          | [02-capsule-and-capability.md](./core/02-capsule-and-capability.md) |
| Capability                  | 不可伪造的权限句柄，绑定对象和操作                                          | [02-capsule-and-capability.md](./core/02-capsule-and-capability.md) |
| Service Graph               | 服务发现、版本协商、启动和系统组织模型                                      | [03-service-graph.md](./core/03-service-graph.md)                   |
| Object Store / Stream       | 持久对象、元数据、索引、版本和流式 IO                                       | [04-data-and-filesystem.md](./core/04-data-and-filesystem.md)       |
| Pager-backed Memory Object  | 用户态 Pager 与内核 VM 协作的可分页内存对象                                 | [05-pager-and-memory.md](./core/05-pager-and-memory.md)             |
| Compute Domain              | 异构计算资源、执行等级和功耗预算模型                                        | [06-compute-and-scheduling.md](./core/06-compute-and-scheduling.md) |
| Hardware Core / Driver Host | 最小硬件仲裁层与用户态驱动宿主                                              | [07-driver-and-kernel.md](./core/07-driver-and-kernel.md)           |
| Communication Fabric        | Portal、Operation、Continuation、EventPort、SharedQueue、Fence 等通信原语族 | [08-communication-fabric.md](./core/08-communication-fabric.md)     |
