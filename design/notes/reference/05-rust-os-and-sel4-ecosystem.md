...
`rust-sel4` 面向的是“在 seL4 userspace 中写 Rust 程序”，而 Ousia 当前是在实现自己的 seL4-like kernel 和 OSTD。它的很多 crate 需要 seL4 构建产物、libsel4 headers、Microkit SDK 或 capDL 环境变量。
...
当前项目已把 `ostd/src/arch/aarch64/cpu.rs` 的手写 `wfe` 汇编替换为 `aarch64_cpu::asm::wfe()`。
...
当前项目已把 `ostd/src/arch/x86_64/cpu.rs` 的手写 `hlt` 汇编替换为 `x86_64` crate 的 `instructions::hlt()`。
...
建议在下一步重写 AArch64 PL011 early console 时引入它。当前 PL011 仍是手写 offset + volatile write，这正是它该替换的位置。
...
- 其 OSTD 的对象假设和 Ousia 的 seL4-like microkernel 不完全一致。
- 整包引入会把架构边界变成外部项目边界。
...
| `tock-registers` | 下一步直接依赖 | 适合替代手写 MMIO register offset |
...
1. 用 `tock-registers` 重写 PL011 early console，顺便排查当前 AArch64 QEMU serial 无输出问题。
2. 从 `rust-sel4/crates/drivers/pl011` 对照 PL011 初始化序列，确认 QEMU virt UART 配置差异。
