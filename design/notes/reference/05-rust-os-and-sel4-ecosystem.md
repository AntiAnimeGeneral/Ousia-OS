...
> 状态：参考材料。本文用于梳理 Rust 社区中可用于 Ousia OS 的 OS/kernel/no_std/seL4 相关库，并区分可直接依赖、可拆参考和暂不采用的组件。规范性设计见 [02-engineering.md](../../topics/02-engineering.md) 与 [04-driver-and-kernel.md](../../core/04-driver-and-kernel.md)；相关 seL4/Microkit 事实见 [03-sel4-mcs-microkit-roadmap.md](./03-sel4-mcs-microkit-roadmap.md)。
...
`rust-sel4` 面向的是“在 seL4 userspace 中写 Rust 程序”。Ousia 当前路线已经转向原生高级 capability kernel；`rust-sel4` 仍适合参考 capability discipline、Microkit/sDDF 用户态系统组织和部分驱动/共享内存组件，但不适合作为 Ousia kernel 内部库。它的很多 crate 需要 seL4 构建产物、libsel4 headers、Microkit SDK 或 capDL 环境变量。
...
- 其 OSTD 的对象假设和 Ousia 的 Phase 1 高级 capability kernel 不完全一致。
- 整包引入会把架构边界变成外部项目边界。
