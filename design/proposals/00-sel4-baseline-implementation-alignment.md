...
当前剩余偏离集中在过渡 owner 尚未闭环：CNode cap 已不再携带 window 起点，但 CNode object 还未真正拥有 CTE array；ObjectTable/ThreadTable 仍是 Vec-backed runtime namespace 和线程状态 owner 分叉；Endpoint/Notification 仍用本地 FIFO entry 而不是真正的 TCB embedded queue links；typed object storage、Reply/TCB commit plan 和部分 executor preflight 返回值仍需要继续收紧。seL4 core 不用通用 map 表达这些核心关系，而是使用 CNode 连续 CTE slot、slot 内嵌 MDB link、TCB 内嵌队列 link、固定 ready queue 数组和 bitmap。
...
- `CNodeCap` 已只携带 guard/radix cap 语义；CNode window 起点由 CSpace typed object payload 持有，普通 initial cap 插入不再为 CNode 暗补默认 window，bootstrap CNode 必须显式注册 window metadata。Runtime CNode object 仍记录 retype radix、logical slot count 和 planned window start，retyped CNode 可以通过 path invocation 使用自己的 reserved CTE window；但它还不是 object-owned CTE array，后续仍需把 CSpace CNode payload 继续迁到 CNode object slot storage。
...
1. CNode object-owned CTE array：让 CNode object 拥有 CTE slot storage，把当前 CSpace CNode payload window metadata 收敛为真实 slot storage，并收紧 lookup 返回值以减少 executor/CSpace 重复 lookup。
...
1. CNode object-owned CTE array：把当前 CSpace CNode payload window metadata 迁到 CNode object slot storage，收紧 lookup 返回值，减少 executor/CSpace 重复 lookup。
