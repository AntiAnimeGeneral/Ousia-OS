...
> 历史 proposal packet。本文曾用于交接 seL4 baseline 对齐实现，当前已被 [Ousia Kernel Architecture Baseline](../implementation/00-ousia-kernel-architecture.md) 取代。本文只作为历史设计记录和被拒绝方向参考，不再作为 implementation handoff 或长期规范源。

> 以下内容为历史原文摘要，保留用于理解被取代路线；不得作为当前实施要求执行。
...
- 历史 owning doc：原 `design/implementation/00-sel4-baseline-rust-replica.md` 已由 `design/implementation/00-ousia-kernel-architecture.md` 取代。
