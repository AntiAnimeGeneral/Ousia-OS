...
本提案的历史起点是：`CapabilitySpace` 曾使用 `HashMap`/`HashSet` 表达 slot、对象和派生关系；`ObjectTable`、`ThreadTable`、scheduler 和 Endpoint/Notification 等待队列也曾依赖通用 map/queue 表达核心事实。前几轮 implementation slice 已经移除 `kernel` core 的 `hashbrown` 依赖，并把这些结构推进到 indexed CTE storage、Vec-backed runtime storage、本地 FIFO wrapper、fixed scheduler topology 和 Endpoint/TCB metadata ownership 的过渡状态。

当前剩余偏离不再是“从 HashMap/VecDeque 起步”，而是过渡 owner 尚未闭环：CNode cap window 起点仍是长期风险，CNode object 还未拥有 CTE array；ObjectTable/ThreadTable 仍是 Vec-backed runtime namespace 和线程状态 owner 分叉；Endpoint/Notification 仍用本地 FIFO entry 而不是真正的 TCB embedded queue links；typed object storage、Reply/TCB commit plan 和部分 executor preflight 返回值仍需要继续收紧。seL4 core 不用通用 map 表达这些核心关系，而是使用 CNode 连续 CTE slot、slot 内嵌 MDB link、TCB 内嵌队列 link、固定 ready queue 数组和 bitmap。
...
本提案已有多轮 implementation slice 落地，接手 agent 不应把下列内容当作未开始任务，也不应回退到旧 map/queue 形态：
...
- CNode path lookup 已具备 root CNode、guard、radix、depth、remaining bits、lookup fault 和 slot window validation shape。`KernelState` 已有 path-based CNode copy/mint/move/delete/revoke invocation；旧 direct descriptor 入口只应视为迁移 facade。
...
- Runtime CNode object 已记录 retype radix 和 logical slot count，证明 CNode backing object 不再只是 marker；但它还不是 object-owned CTE array，后续仍需把 CNode cap 的 window 过渡形态迁到 CNode object slot storage。
- Endpoint sender queue 已不再保存完整 `IpcMessage`。queue entry 只保存 sender/thread CPU identity；`ThreadState::BlockedOnSend` 拥有 sender CPU、badge、grant、grant-reply、call flag 和 payload。`IpcPayload` 已移入 `kernel::message`，避免 TCB 依赖 endpoint implementation。
- Endpoint receiver queue 已不再保存 receive grant。queue entry 只保存 receiver/thread CPU identity；`ThreadState::BlockedOnReceive` 拥有 receive grant 和 receiver-side reply destination。send-side call reply setup 和 reply cap grant 必须从 TCB receive state 重建。
- `design/implementation/00-sel4-baseline-rust-replica.md` 已同步记录 CNode path/window、CNodeObject metadata、Endpoint sender/receiver queue identity-only 过渡状态，以及仍需迁到真正 TCB embedded queue links 的事实。

最近相关提交可作为切片边界参考：`2f067ba`、`93cfca2`、`b3b4558`、`64c8767`、`4d9171e`、`dc1d7c8`。

这些切片完成时已按改动范围运行过对应验证：`cargo fmt --check`、`cargo check -p kernel`、`cargo test -p kernel`，涉及 design doc 更新时运行 `deno task --cwd .github/skills/doc-validation check:docs --config ../../../design/check-docs.config.json`。最后两个 Endpoint/TCB ownership 切片还经过 `代码实现 + diff` 黑队审查；审查重点覆盖 endpoint 不再拥有 blocked send/receive metadata、TCB state 作为权威、failure-before-side-effect 和测试是否保护真实语义。
...
- 测试应保护语义而不是构造器字段。新增或迁移测试优先覆盖 public/executor 路径、失败无副作用、queue mismatch preflight 和 owner state 不变；只改 expected struct 字段不足以证明迁移正确。
...
1. 已完成：完成 Gate 0 的核心方向确认。当前 baseline variant 采用 classic/non-MCS seL4；scheduler shape 已具备 fixed CPU topology、ready lane array 和 bitmap；CNode addressing 已具备 path lookup API shape；typed object storage 仍是剩余重点。
2. 已部分完成：引入 CTE/CNode 目标结构和 CSpace lookup path。现有 descriptor 测试已经通过 CNode path/window 逐步迁移；剩余工作是让 CNode object 自身拥有 CTE slot array，删除 CNode cap window 起点作为长期事实源。
3. 已部分完成：把 capability insert/copy/mint/move/delete/revoke 改成操作 indexed CTE storage 和 MDB/lineage metadata。剩余工作是进一步删除旧 direct descriptor facade 和任何仍像全局 slot/object map 的派生事实。
4. 已部分完成：Untyped/retype 已支持 source Untyped、target slot window、object kind/count/size、alignment、watermark 和 CNode window bounds preflight。剩余工作是把模型 object size table 替换为真实 typed object layout/CNode size policy，并让 perform 阶段只消费已验证 backing memory 和 CTE slots。
5. 剩余重点：替换 `ObjectTable`。把 Endpoint/Notification/Reply/TCB/CNode/Frame backing state 迁到 typed object storage，并确保 cap object ref 与 backing object 一致；不要继续让裸 `ObjectId` 绕过 authority lookup。
6. 剩余重点：统一 TCB object 与 ThreadTable。TCB object 应成为 thread state owner；删除 ObjectTable TCB binding 与 ThreadTable TCB state 的长期双写路径。
7. 已部分完成：Endpoint queue 已只保存 sender/receiver thread+cpu entry，blocked send/receive metadata 已迁入 TCB state。剩余工作是把本地 FIFO entry 改成真正 TCB embedded queue links，让 Endpoint 只持 head/tail，并覆盖 cancel/dequeue mismatch 失败无副作用。
8. 剩余重点：将 Notification queue 改成 TCB embedded queue links，覆盖 active badge OR、bound TCB receive、wait/poll、cancel 和 signal/wait mismatch 失败无副作用。
9. 剩余重点：将 Reply pending state 接入 TCB/Reply object owner，确保一次性 reply cap 消费、pending caller、caller TCB state 和 scheduler wakeup 在同一 commit plan 中完成。
10. 已部分完成：scheduler 已使用 fixed CPU topology、per-CPU ready lane array 和 bitmap。剩余工作是继续扩展 priority/domain selector 语义并去掉任何会让调度退回 FIFO-only 的 facade。
11. 剩余重点：更新 `KernelState::execute_invocation` 为更严格的 decode/preflight/perform 结构，删除旧临时 adapter，并消除检查阶段和提交阶段对同一事实的重复 lookup。
12. 已完成：删除普通 kernel 主路径中的 `hashbrown` 依赖。若后续测试或 host-only helper 需要 map，必须放在 `#[cfg(test)]` 或非 core 路径并说明边界。
13. 持续执行：更新 `design/implementation/00-sel4-baseline-rust-replica.md` 的实现路线和当前运行路径，记录新的 owner、剩余 adapter 和验证命令。
...
迁移时可以暂留 compatibility facade，但它们只允许作为短期脚手架，不能成为设计约束。每个 facade 必须有删除条件和退出触发点；如果某个 facade 迫使新代码重复 lookup、双写状态、维持错误 owner 或绕开最终数据结构，应优先删除或改写它，而不是继续兼容。例如 `CapabilityDescriptor` facade 的删除条件是 CNode path descriptor 完整接管测试和 executor；`ObjectTable` adapter 的删除条件是 typed object storage 覆盖所有 backing object lookup。
...
当前交接状态：本提案已进入 implementation 阶段，前几轮切片已完成 CSpace indexed storage、CNode path/window、Untyped path retype、CNode runtime metadata、Endpoint send/receive metadata 迁入 TCB state。下一位 implementation agent 应从剩余长期事实源继续推进，而不是重新讨论是否允许破坏性重构。

建议下一批切片按以下顺序选择，除非代码现状显示更强的局部依赖：

1. CNode object-owned CTE array：把 CNode cap 的 window 起点过渡形态迁到 CNode object slot storage，收紧 lookup 返回值，减少 executor/CSpace 重复 lookup。
...
4. Typed object storage：替换当前 Vec-backed ObjectTable runtime namespace，让 retype commit plan 创建 backing object，并让 cap object ref 成为 lookup 入口。
...
- 已复核本文 `已完成进度` 和 `实施步骤` 状态，不重复实现已经落地的切片。
- 已确认 baseline variant、CNode addressing、scheduler shape 的当前实现方向；typed object storage 仍是未完成决策与实现重点。
