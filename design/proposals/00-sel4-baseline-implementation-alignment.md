...
- `CNodeCap` 已只携带 guard/radix cap 语义；CNode window 起点由 CSpace object metadata 持有，Runtime CNode object 仍记录 retype radix、logical slot count 和 planned window start，retyped CNode 可以通过 path invocation 使用自己的 reserved CTE window；但它还不是 object-owned CTE array，后续仍需把 CSpace window metadata 继续迁到 CNode object slot storage。
...
1. CNode object-owned CTE array：让 CNode object 拥有 CTE slot storage，把当前 CSpace object window metadata 收敛为真实 slot storage，并收紧 lookup 返回值以减少 executor/CSpace 重复 lookup。
...
1. CNode object-owned CTE array：把当前 CSpace object window metadata 迁到 CNode object slot storage，收紧 lookup 返回值，减少 executor/CSpace 重复 lookup。
