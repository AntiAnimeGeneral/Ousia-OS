...
6. 重新实现正确性纪律：不可伪造 handle、rights 单调性、stale handle 明确失败、失败前置检查、commit 无可恢复失败、显式 enum state machine、多核 scheduler 假设。
...
- Scheduler 的 multi-core-only 假设、per-CPU run queue、ready lane 和重复 enqueue 防护必须重新实现。
...
保留当前 scheduler 的多核 shape，但修正存储和 lookup：
...
- Cross-core wake、timer routing、TLB shootdown 先用 explicit boundary placeholder，不能假设 single-core。
