...
2. 继续收敛 CSpace descriptor facade、CNode window metadata 和 ObjectTable runtime CNode metadata。Path-based CNode executor、Untyped retype commit plan 和 MDB/child lineage 已消费 resolved CTE handle；下一步应把 remaining raw descriptor/non-path API 和 reply cap target shaping 继续迁到 CTE-owned 事实，不让全局 slot/window 事实重新成为内部权威。
