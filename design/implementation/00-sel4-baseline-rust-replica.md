...
剩余工作集中在更完整的 seL4 CSpace/Untyped 细节：继续收敛 descriptor facade 与 runtime ObjectTable 过渡事实、Untyped 可用区间、watermark、alignment、容量 accounting、完整 object size table，以及 revoke 与并发 IPC/多核调度之间的一致性。CNode CTE slot storage 已进入 CNode typed object payload，后续重点是把 CSpace/ObjectTable 的 runtime object metadata 继续迁向真实 typed backing storage。
...
2. 继续收敛 CSpace descriptor facade、CNode window metadata 和 ObjectTable runtime CNode metadata。Path-based CNode executor、Untyped retype commit plan、MDB/child lineage、reply cap install target 和 receiver-side reply slot 已消费 object-owned CTE handle；下一步应把 remaining raw descriptor/non-path API 继续迁到 CTE-owned 事实，不让全局 slot/window 事实重新成为内部权威。
...
4. 继续清理核心对象路径里的动态容器偏离；Endpoint/Notification 的等待关系已经迁到 TCB embedded queue link，Endpoint/Notification 只保留 head/tail/count anchor。
...
- `kernel::cap` 的 CSpace-like model 已把 seL4 CNode/TCB 的基础 slot 操作拆开，并把 slot 事实收敛到 indexed CTE storage：`copy` 只降权并继承已有 badge；`mint` 在不扩权的前提下允许 Endpoint/Notification 设置新 badge；`move_capability` 转移 CTE slot 内容并维护 CTE-ref MDB/派生关系，不把 move 误建模成新的派生。旧 `derive` 只是兼容入口，语义收敛到 `copy`。CNode path lookup 已具备 root CNode、guard、radix、depth、remaining bits 和 lookup fault shape，`CNodeCap` 只携带 guard/radix cap 语义；目标 CTE slot 由 CNode typed object payload 拥有的 slot array 按 radix offset 解析，lookup 返回值携带内部 CTE location，source/target/destination descriptor 只作为外部 facade。TCB payload 拥有 dedicated reply CTE；该 slot 不能被 raw `copy_into`/`mint_into`/retype destination、CNode window 或普通 free-list 分配复用，只能由 receiver TCB reply install 路径解析并复用。初始 CNode 必须通过 CNode-specific bootstrap 入口显式注册 CTE slot window；后续普通 initial cap 装入当前 bootstrap CNode 的真实 CTE slot，而不是创建 root-level shadow slot。`KernelState` 的 path-based CNode copy/mint/move/delete/revoke invocation 通过 source/target/destination CNode path 授权，并把 resolved CTE handle 交给 CSpace commit API；raw descriptor API 仍作为外部/test facade 保留。`retype_untyped` 只允许 Untyped cap 创建新的 child object，并检查目标对象最小大小不超过源 Untyped；当前覆盖 Endpoint、Frame、CNode、Untyped、TCB 和 Notification。CNode retype 会预留 CNode-owned CTE slot array，并保留 `window_start` 作为短期 descriptor facade/debug metadata，而不是作为内部 slot storage 事实；retype commit 在分配 TCB 内部 CTE handle 前先推进 planned destination/window slot 边界，避免 TCB reply slot 与 retype 目标窗口碰撞。Reply cap 由 call/reply 路径创建，`install_reply_capability` 把一次性 cap 安装进 receiver TCB-owned reply CTE，不分配新的 backing Reply object 或自动全局 slot，避免 CSpace metadata、ObjectTable Reply state 和 reply target slot 分叉。当前 retype 已建模 CTE-ref MDB lineage、slot window preflight、模型 object size table、alignment、watermark、容量 accounting 和 resolved CTE 提交计划；这些 size 常量用于防止零大小对象和验证事务语义，不代表冻结后的 seL4 object size ABI，后续应由真实 typed object layout、CNode radix/guard policy 和 frame metadata 替换。Reply cap 仍不可派生，避免复制一次性 reply 权限。CSpace 的 retype plan 保存 resolved source/destination CTE，让 executor 在提交需要 ObjectTable runtime entry 的 retype 前预检真实对象表绑定，避免 CSpace 和 ObjectTable 之间出现半事务；nested Untyped 当前只提交 CSpace lineage，不写 ObjectTable，但会消耗 parent Untyped capacity 并获得独立 allocation state。
- `kernel::ipc` 承载 seL4 baseline Endpoint 的最小状态机：`ipc/mod.rs` 是 facade，`ipc/endpoint.rs` 拥有 Endpoint `Idle / Send / Recv` 状态和 sender/receiver wait queue head/tail/count，`ipc/message.rs` 拥有 `IpcPayload`、`IpcError` 和 message word 上限。Endpoint 不再保存本地 FIFO entry 或 blocked IPC metadata；生产路径由 `thread::action` 读取 Endpoint head、从 TCB state 重建 sender/receiver CPU、badge、grant、grant-reply、call、payload 和 receiver-side reply destination，并通过 TCB-owned embedded wait links 执行入队、出队、unlink 和 drain。blocking send/receive 会先完成 current-thread、reply、scheduler 和 receiver/sender state preflight，再提交 Endpoint anchor、TCB state/link 和 scheduler mutation；nonblocking send 在没有 receiver 时不入队，nonblocking receive 在没有 sender 时失败返回。reply cap destination 属于 receiver-side receive context；Endpoint queue 不直接拥有 CSpace slot。`Endpoint::send`/`recv` 只保留为局部状态机兼容/test facade，不能作为跨 TCB/Scheduler/Reply 的生产事实源。
- `kernel::notification` 承载 seL4 baseline Notification 的最小状态机：Notification 显式使用 `Idle / Waiting / Active` 三态。idle/active signal 在无等待线程时按 OR 语义累积 active badge，idle 且绑定的 TCB 正在等待 receive 时返回 bound receive completion；active wait 消费 badge；poll 在没有 active badge 时失败返回且不阻塞。Notification 不拥有 scheduler，也不直接读取 TCB 状态；TCB/调度层判断 bound TCB 是否能接收后，把条件传入 notification 边界。Notification wait queue 已迁到 seL4-style TCB embedded membership：Notification 只保存 waiter head/tail/count anchor，`ThreadState::BlockedOnNotification` 拥有 notification object 和 receiver CPU metadata，TCB `TcbWaitQueueLink` 拥有 prev/next membership。`thread::action` 在 wait/signal/cancel/finalise 生产路径中先完成 current-thread、TCB state 和 scheduler wake preflight，再提交 Notification anchor、TCB state/link 与 scheduler mutation；object-local `Notification::wait`/`signal` 只保留非 waiting delivery 的局部状态机 facade，不能作为跨 TCB/Scheduler 的生产事实源。
- `kernel::reply` 承载 seL4 baseline Reply 的最小状态模型：Reply 保存至多一个 pending caller，reply 成功后消费这个 pending state 并返回需要唤醒的 caller 信息。Reply cap 当前携带 caller object、target object 和 receiver-derived grant 语义；普通 Reply cap 不可派生，避免复制一次性 reply 权限。CSpace 提供 `consume_reply_cap`，只允许 Reply cap 通过并删除对应 slot；Endpoint call executor 在 immediate delivery 和 queued receive 两条路径上创建一次性 Reply cap 时，必须从 receiver ThreadId 反查 ObjectTable TCB object，并把 cap 安装进该 TCB payload 拥有的 dedicated reply CTE，而不是自动分配全局 slot 或消费调用上下文伪造的 CTE token。KernelState 的 Reply executor 先校验 Reply cap metadata 与 Reply pending state 的 caller、target 和 grant 语义一致，再执行 `reply_to_caller`，并在成功后消费 reply cap slot。Reply 不拥有 scheduler，也不直接复制消息寄存器；消息寄存器和完整 cap transfer 仍留给后续 syscall ABI/CNode/TCB backing 阶段。
- `kernel::thread` 是线程相关 owner 的目录层次：`thread/tcb.rs` 承载 seL4 baseline thread identity、thread state baseline 和 endpoint/notification wait queue embedded link；`thread/action.rs` 承载跨 TCB、Endpoint、Notification、Reply 和 Scheduler 的线程事务。`Inactive`、`Running`、`Restart`、`BlockedOnReceive`、`BlockedOnSend`、`BlockedOnReply`、`BlockedOnNotification` 和 `IdleThreadState` 显式建模，并提供 blocked/stopped 判定。`BlockedOnSend` 保存 endpoint、sender CPU、badge、grant、grant-reply、call 和 payload；`BlockedOnReceive` 保存 endpoint、receiver CPU、receive grant 和可选 receiver-side Reply object destination；`BlockedOnNotification` 保存 notification object 和 receiver CPU。Endpoint/Notification queue membership 由 TCB `TcbWaitQueueLink` 的 prev/next 拥有，对象只保存 head/tail/count；finalise/cancel/delete 会在移除 TCB 前通过 link 摘除 endpoint/notification queue entry，避免悬挂 waiter。TCB configure 的首版事务只把未绑定 TCB object 绑定到新的 inactive `ThreadId` 和 affinity，不自动入队；TCB resume 只允许 `Inactive -> Restart` 并按 affinity 入队。TCB 持有可选 bound notification 关系，并提供 `waits_on_bound_notification_receive` 这类由 TCB 状态推导出的查询；Notification 对象只消费查询结果，不直接拥有或读取 TCB 状态。`CpuId`/`ThreadId` 属于 TCB/调度边界，不属于 IPC 私有类型；TCB affinity 从一开始显式存在，后续 scheduler 按多核语义使用它。
- `kernel::object` 是 Phase 1 的 runtime object storage 边界：CapabilitySpace 保存 authority、CTE/MDB lineage、object id、generation 和 typed object payload；CNode payload 已拥有 CTE slot array。ObjectTable 保存 Endpoint、Frame、Notification、Reply 实体，保存带 radix/slot-count/window-start metadata 的 CNode runtime object，并保存 TCB object id 到可选 ThreadId 的绑定。ObjectTable 当前是 bounded slot storage，初始化时分配固定容量，Untyped retype 在 CSpace commit 前批量预检 runtime object slot 容量，提交阶段不再依赖 `Vec::push` 或隐式分配；绑定事实扫描和 object-id 线性 lookup 仍是过渡偏离。后续 typed backing storage 仍需把 Endpoint/Notification/Reply/TCB/CNode/Frame state 迁到由 capability object ref 指向的 backing object，并最终消除 ObjectTable runtime namespace。FrameObject 当前只保存最小 `size_bits` metadata，不保存 cap rights、mapping state、page-table ownership 或 physical frame allocator 事实；rights 仍由 Frame cap/CapabilitySpace 拥有。CNodeObject 当前记录 retype radix、逻辑 slot 数和 planned window start；这些字段是 runtime/debug facade，CSpace CNode payload 才是 CNode path lookup 与 CTE mutation 的 slot array 事实源。未绑定 TCB object 是 retype 后、配置/绑定前的合法状态；TCB object id 和 ThreadId 不允许互相假设相等。Endpoint call 记录 Reply pending caller 时必须经 ObjectTable 反查真实 caller TCB object。TCB 实体仍由 `thread::action::ThreadTable` 单一拥有，避免 ObjectTable 和 ThreadTable 双写线程状态；ThreadTable 当前是 bounded slot storage，初始化时分配固定容量，TCB configure/insert 在 ObjectTable binding 前预检容量，提交阶段不再依赖 `Vec::push` 或隐式分配；线性 lookup 仍是过渡偏离，最终应由 TCB object/backing reference 和已解析 cap/object 指针替代。ObjectTable 统一维护 object id 绑定和对象类型检查；这让 executor 可以在不使用 unsafe 的前提下同时取得 Endpoint 和 Reply 的 mutable reference，用于 call/reply 事务提交。
- `kernel::scheduler` 使用 fixed CPU topology 的 per-CPU run queue vector；每个 CPU queue 拥有 current thread、ready lane array 和 non-empty bitmap。ready lane 已改为 bounded ring storage，enqueue/pop/remove 不再依赖 `Vec::push`、`Vec::remove(0)` 或动态扩容；ready queue full 映射为资源不足边界。第一版只启用一个 priority/domain lane，但 enqueue、schedule、yield 和 remove 都通过 selector/bitmap 形状进入 ready queue，不暴露长期 FIFO-only scheduler API。CPU topology 的 `run_queues` 仍是初始化期 vector，后续可继续收敛到 bounded topology storage。
- `kernel::state` 是 Phase 1 的 invocation executor 骨架：KernelState 统一持有 CapabilitySpace、ObjectTable、ThreadTable 和 Scheduler。`InvocationContext` 显式携带当前线程、当前 CPU、IPC payload 和可选 receiver-side Reply object；reply cap 目标 CTE 由 executor 通过 receiver ThreadId 解析到 ObjectTable TCB object，再交给 CapabilitySpace 的 TCB-owned reply CTE install 路径，避免从薄参数里猜测或信任外部传入的 reply cap 目标 CTE。`execute_invocation` 先调用 `kernel::invocation` 做 capability type/right 授权，再按授权结果查找真实对象并分派到 `kernel::thread::action` 事务入口。当前 executor 已覆盖 Endpoint send/recv、Notification signal/wait、Reply、TCB configure/resume、path-based CNode copy/mint/move/delete/revoke，以及 Untyped retype 到 Endpoint/Frame/CNode/Notification 的最小真实对象创建、nested Untyped 的 CSpace-only 创建和 TCB 的未绑定 object 创建；Endpoint call 成功创建 Reply cap 时通过 `ThreadWithReplyCap` 返回新 descriptor，Endpoint/Frame/CNode/Notification/nested Untyped/TCB retype 成功时通过 `Retyped` 返回新 descriptor。TCB retype 只创建 ObjectTable 中的未绑定 TCB object，不创建线程、不设置 affinity，也不触碰 scheduler；TCB configure 在检查 object 未绑定、ThreadId 未存在且 affinity CPU 已知后，绑定 ObjectTable 并插入 inactive TCB，但仍不自动入队。CNode retype 创建带 radix 和 planned window metadata 的 CSpace/ObjectTable object 记录，并让 retyped CNode 可以通过 path invocation 使用 CSpace payload 拥有的 CTE slot array；Frame retype 当前只建立 runtime FrameObject metadata；Frame map 仍返回明确 unsupported outcome，直到 page-table ownership、address-space owner 和 mapping failure 事务边界接入。
