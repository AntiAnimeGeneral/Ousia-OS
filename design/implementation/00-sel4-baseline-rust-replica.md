...
2. 继续收敛 CSpace descriptor facade、CNode window metadata 和 ObjectTable runtime CNode metadata。Path-based CNode executor 已消费 resolved CTE handle；下一步应把 remaining raw descriptor/non-path API 和 retype destination commit 继续迁到 resolved CTE，不让全局 slot/window 事实重新成为内部权威。
