...
当前 `kernel/src` 中的 CSpace-like capability、Endpoint、Notification、Reply、TCB、ObjectTable、Scheduler 和 KernelState 是重要 prototype evidence：它们已经验证了 capability rights、stale descriptor、失败前置检查、thread action 和 scheduler mutation 的若干不变量。

这些实现不再被要求继续向完整 seL4 baseline 收敛。后续重构应判断每个结构应去向：

- 保留：rights 单调性、generation/stale handle、失败无部分提交、typed object check、thread/scheduler transaction discipline。
- 演进：`CapabilitySpace` 转向 per-process handle table / object manager；Endpoint/Reply 转向 channel/call；ObjectTable 转向 typed kernel object storage；Frame/MemoryObject 转向 VM subsystem。
- 停止模仿：裸 CNode/CSpace public API、Untyped/retype 作为普通用户库资源申请模型、seL4-specific error ordering、MDB 作为产品 API。

## 近期代码步骤

1. 定义 `Handle` / object manager 的 owning module 和 process-local handle table 方向，明确哪些现有 `cap` API 是兼容 facade，哪些是内部 substrate。
2. 设计 kernel object trait/enum 边界，先覆盖 channel、event、process、thread、memory object 和 namespace node。
3. 设计 VM/page allocator/kernel heap 边界：boot frame metadata、page allocator、slab/fixed pool、allocation context 和 failure preflight。
4. 设计 channel/call 的 message bytes + handle transfer model，并保留同步 call 与异步 wait 两种用户库入口。
5. 设计 Object Namespace/VFS/Object Store 的内核/服务放置 proposal，比较 kernel-resident、user-service 和 hybrid 三种方案。
6. 更新 tests：从 seL4 helper 复述转向 Ousia handle/object/channel/VM/VFS 行为契约。
7. 保持 QEMU AArch64 smoke 与 host tests 双线验证；涉及 Rust source 时按 workflow 运行 `cargo fmt --check`、`cargo check` 和 targeted tests。
