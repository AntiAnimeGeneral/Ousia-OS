...
- 从一开始按 always-multicore native HMP、失败无部分提交和热路径分配约束设计。单核不是支持目标，也不能作为 correctness 或 performance 论据；SMP/同构多核只是退化情况；大小核、GPU/NPU/DSP/SmartNIC、共享内存带宽、设备本地内存和功耗域都属于同一套资源模型。
...
当前 handle/object 主线由 `handle::HandleTable`、`object::ObjectManager` 和 `process::Process` 承载：

- 外部 facade 是不可伪造 `HandleValue`，携带 process-local slot index 与 generation 以拒绝 stale handle。
- `Process` 拥有 handle table、resource budget 和 syscall-facing authority boundary；`ObjectManager` 拥有 object id、kind、generation、payload、lifetime 和 handle count。
- rights 只能在 duplicate/transfer/install 边界收缩或保留，不能从 object 或 syscall descriptor 凭空扩权。
- handle transfer 通过 channel/call、Portal 或显式 broker 边界完成；权限检查集中在 syscall/object boundary。
- 旧 `SlotId`、CNode window、Untyped retype 和 MDB-like lineage 只允许作为 Git 历史或 seL4 reference 中的安全算法证据，不再保留为实现树概念或兼容 facade。
...
线程和调度继续保持 native HMP 主路径：

- Process/Capsule 拥有 address space、handle table、thread set 和 resource budget。
- Thread 拥有 register context、affinity、execution class、blocking state 和 wait membership。
- Scheduler 拥有 per-CPU/per-compute-domain run queues、current execution context、priority/domain lanes、cross-core wakeup、TLB shootdown 和 power/thermal feedback 边界。
- Compute Domain 把同构 CPU、多 cluster 大小核、GPU/NPU/DSP/SmartNIC、硬件队列、设备本地内存、共享内存带宽和功耗域纳入同一资源模型。调度器不能只按 CPU runnable queue 思考；它必须能表达“任务适合哪个执行后端、数据在哪里、迁移成本多少、功耗预算是否允许”。
- 所有调度和等待路径的性能判断都必须以并发 runnable、跨核唤醒、跨资源队列竞争和 HMP placement 为主路径；不能用单核 smoke path 证明设计成立。
- 早期 QEMU smoke 可以覆盖小实现面，但设计不能支持或假设 single-core，也不能把同构 SMP 当成最终硬件模型。
...
当前 `kernel/src` 的实现树已经切到 Ousia-native skeleton：`error`、`handle`、`object`、`process` 和 `syscall`。旧 CSpace-like capability、Endpoint、Notification、Reply、TCB、ObjectTable、Scheduler 和 KernelState prototype 已从实现树移除，避免继续被误读为兼容入口或迁移底座。需要审查历史语义时只回看 Git 历史、owning docs 和 `third_party/**` reference；新代码不得重新依赖旧 module shape。
...
- 旧 CNode path、Untyped retype、seL4-specific invocation variants、旧 ObjectTable 和旧 executor tests 已从主线实现和测试约束中移除。

旧 prototype 只继承可证明仍成立的语义，不继承源码形状：

- 保留：rights 单调性、generation/stale handle、失败无部分提交、typed object check、thread/scheduler transaction discipline。
- 当前替代：`CapabilitySpace` 的 authority 语义由 per-process handle table / object manager 承接；Endpoint/Reply 产品主线由 channel/call 承接；Frame/MemoryObject 资源语义由 MemoryObject / AddressSpace / 后续 VM subsystem 承接。
...
4. 将 `AddressSpaceObject` 从固定 mapping slots 演进到可保留 metadata/page-table resources 的 exclusive VM reservation token，明确 page table owner、TLB shootdown、HMP locking 和 pager fault 边界。
