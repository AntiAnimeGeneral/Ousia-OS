...
- `syscall::Kernel` 提供最小 decode/preflight/commit 边界，先覆盖 bootstrap process、object handle creation、handle duplicate 和 close。
...
1. 扩展 Slice 1 skeleton 的 object kind state，使 `ChannelEndpoint`、`Event`、`MemoryObject`、`AddressSpace` 和 `Thread` 不只是名字，而有最小 owner state、operation 和失败边界。
2. 裁决 handle revoke lineage 的第一版结构，比较 parent pointer、reverse index、generation domain 和 compact derivation tree 的 lookup/revoke 成本。
3. 设计 resource reservation token，使 object creation、handle installation、queue entry、VM mapping 和 namespace cache 的失败都发生在 commit 前。
4. 读取本地 Zircon/Fuchsia channel/call 源码，记录采用、调整和拒绝点，然后实现 bounded bytes + handle transfer 的 channel vertical slice。
5. 读取本地 Zircon/Fuchsia VMO/VMAR/address-space 源码，记录采用、调整和拒绝点，然后实现 MemoryObject create/map/unmap skeleton。
6. 设计 Object Namespace/VFS/Object Store 的内核/服务放置 proposal，比较 kernel-resident、user-service 和 hybrid 三种方案。
7. 保持 QEMU AArch64 smoke 与 host tests 双线验证；涉及 Rust source 时按 workflow 运行 `cargo fmt`、`cargo fmt --check`、`cargo check` 和 targeted tests。
