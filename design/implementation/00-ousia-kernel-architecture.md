...
- `syscall::Kernel` 提供最小 decode/preflight/commit 边界，先覆盖 bootstrap process、object handle creation、MemoryObject creation、handle duplicate 和 close。
...
1. 裁决 handle revoke lineage 的第一版结构，比较 parent pointer、reverse index、generation domain 和 compact derivation tree 的 lookup/revoke 成本。
2. 设计 resource reservation token，使 object creation、handle installation、queue entry、VM mapping 和 namespace cache 的失败都发生在 commit 前。
3. 读取本地 Zircon/Fuchsia channel/call 源码，记录采用、调整和拒绝点，然后把 `ChannelEndpointObject` 从 payload skeleton 扩展为 bounded bytes + handle transfer 的 channel vertical slice。
4. 读取本地 Zircon/Fuchsia VMO/VMAR/address-space 源码，记录采用、调整和拒绝点，然后把 `MemoryObject` / `AddressSpaceObject` 从 metadata skeleton 扩展为 create/map/unmap skeleton。
