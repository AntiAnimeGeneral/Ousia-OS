...
- `ostd::mm::frame` 先承载物理页框的基本不变量：页大小、物理地址类型、页对齐区间、单区间和多区间 early frame allocator。它参考 Asterinas early frame allocator 的方向，先把 boot-time frame ownership 从 heap 中分离出来；后续再把 bootloader/device-tree 提供的真实 memory map 归一化成可分配区间，接入 frame metadata 和 seL4-style Untyped 派生。
