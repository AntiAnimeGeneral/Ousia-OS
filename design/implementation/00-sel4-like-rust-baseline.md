...
3. 继续收紧 Untyped 可用区间、watermark、alignment 和容量 accounting；当前代码已有模型 object size table，后续需替换为真实 seL4-like object layout 与 CNode size policy。
...
- `kernel/` 保持为架构无关核心内核库 crate，承担 seL4-like capability / IPC / scheduler / object / invocation / executor 等内核语义。它不拥有 `kernel_main`、panic 策略、OSTD boot wiring、MMIO 寄存器、boot stack 或架构启动汇编。
...
- `kernel::error` 承载 syscall-facing 的稳定错误码折叠：Capability、Invocation、ObjectTable、ThreadAction、Scheduler 和 KernelExecution 的 richer typed errors 会映射到少量 seL4-like `KernelErrorCode`。slot/object/thread 的诊断字段仍保留给模型测试和 debug context，但外部边界只消费 FailedLookup、InvalidCapability、IllegalOperation、InvalidArgument、RangeError 等稳定类别。
