...
本文是 [05-data-and-filesystem.md](./05-data-and-filesystem.md) 与 [06-pager-and-memory.md](./06-pager-and-memory.md) 的深挖材料，不是新的顶层规范。三部分：现代 FS 实践调研 → 目录树问题分析与 Ousia OS 方案 → VM 整合。若本文与主设计冲突，以 05/06 的契约为准。
...
| **KV**     | ❌ 问错了问题 | **配置文件是错误的抽象**。配置管理归属 [15-environment-and-deps.md](./15-environment-and-deps.md) 的类型化配置服务，而不是在 Object Store 里加 KV 类型存配置文件。Object Store 不应因为兼容 `.yaml` / `.toml` / `.env` 而膨胀成通用配置数据库 |
