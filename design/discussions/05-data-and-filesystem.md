...
字节流文件没有结构、索引、版本、关系、事务——现代应用反复在文件系统之上重建这些基础设施（SQLite 是最广泛部署的数据库）。xos 的 Object Store 位于"字节流文件"和"应用数据库"之间：提供基础结构、索引、版本，但不做完整 SQL 引擎。
...
高频元数据操作的挑战：IPC 延迟。对策：元数据推送 + Capsule 本地缓存、批量操作（一次 IPC 返回整个目录）、内核 Object ID 缓存。
