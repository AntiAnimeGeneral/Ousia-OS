...
库依赖冲突：两个 Capsule 需要同一个库的不同版本 → 两个版本同时存在于内容寻址存储，各自挂载到各自的私有命名空间。

用户偏好冲突：用户偏好是 advisory 的，应用可以声明不使用或自行降级。不是 `LANG=zh_CN` 全局覆盖一切。

## 动态库：也是系统组件，需要被管理

**核心立场：库 = Package Cell。** 库和应用是同一种东西的不同用途。库的安装、卸载、依赖管理、版本控制走完全相同的机制。
...
Linux 兼容域（类似 WSL2 的 VM）内部保持传统 `ld.so` 模型。原生 xos 空间不受此影响。兼容层不污染原生抽象。
