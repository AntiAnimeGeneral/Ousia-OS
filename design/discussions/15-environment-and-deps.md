...
>
> 依赖解析、Package Cell 生命周期、多版本并存的权威设计见 [02-package-cell.md](./02-package-cell.md)。本文只讨论 Capsule 启动时环境如何生成，以及系统/用户配置为什么不应退化为 shell 初始化脚本或配置文件碎片。
...
**核心立场：库 = Package Cell。** 库和应用是同一种东西的不同用途。库的安装、卸载、依赖管理、版本控制归属 [02-package-cell.md](./02-package-cell.md)。本文只约束运行环境如何引用这些 Cell。
...
- [02-package-cell.md](./02-package-cell.md) — Cell 的依赖声明
- [03-capsule-and-capability.md](./03-capsule-and-capability.md) — Capsule 沙盒边界
- [13-fs-vm-deep-dive.md](./13-fs-vm-deep-dive.md) — "配置文件是错误的抽象"
- [14-shell-and-tools.md](./14-shell-and-tools.md) — xsh 中的配置命令
