...
Pager-backed Memory Object 是 xos 最关键的底层抽象——它是"用户态文件系统 + 原生虚拟内存"能否成立的支点。本文讨论它的设计、与内核 VM 的交互协议、崩溃模型，以及与传统 `mmap` 的对比。
...
xos 的文件系统在用户态。如果每次缺页都需要 RPC 到用户态存储服务，`mmap` 的性能优势就消失了。
...
对于频繁缺页的场景（如顺序读取大文件），逐个 PageRequest → PageData 的往返开销太大。xos 提供两种优化：
...
业界有些系统（如某些分布式文件系统）支持 Pager 透明备援——一个 Pager 崩溃后，另一个无缝接管。xos 不这样做，原因：
...
如果一个 Capsule 已经完成了 `write()` 等效操作（即存储服务向应用确认了写完成），那么回写必然已经完成——因为**存储服务有责任在确认写完成之前完成回写**。

Pager 崩溃时丢失的，只是"还没有被确认给应用的修改"。xos 接受这个语义——这等价于应用崩溃场景。

---

## 与传统 mmap 对比

| 维度       | 传统 mmap (Linux)   | xos Pager-backed MO          |
| ---------- | ------------------- | ---------------------------- |
...
1. **共享页池的安全边界**：如果 Pager 和 Capsule 共享物理页框，Capsule 能否通过写入这些页来破坏 Pager 的数据？需要什么样的页表权限隔离？
2. **大页支持**：2MB/1GB huge page 是否可以由 Pager 统一管理？缺页处理粒度是否需要从 4KB 提升？
3. **NUMA 感知**：在多 socket 系统上，Pager 需要知道物理页框位于哪个 NUMA 节点，以优化数据放置吗？
4. **异构内存**：如果有 HBM + DDR + CXL 内存池，Pager 如何表达"把热数据放在 HBM 中"？
