...
1. **宿主单元测试**：在宿主 Rust test harness 下验证单一 owner 的本地语义，例如 handle rights、object type、channel queue、VM mapping descriptor、状态 enum 转换和稳定 ABI 编号。它不依赖硬件，也不声称覆盖真实 boot 或 no_std 环境。
2. **宿主集成测试**：在 `kernel/tests/*.rs` 中通过真实 Ousia-native kernel 边界验证跨 owner 协作，例如 `Kernel::execute` 触发 Process handle table、ObjectManager、ChannelEndpoint 和 AddressSpace 的事务性变化。失败路径必须比较相关 owner state，证明 handle/object/message/mapping 没有部分提交。
...
5. **模型和形式化检查**：能力传递不越权、IPC 不重复/不丢失、IOMMU 映射不重叠、缺页处理不泄漏。工具候选包括 TLA+ 和 Verus；第一阶段先形成可机检规约，不追求全系统证明。Asterinas 的 Converos 说明 Rust OS kernel 的并发模块可以用多层、多粒度规格进行实用模型检查；Ousia 应优先把 frame lifecycle、reservation token、channel call wait/wake、handle revoke 和 VM fault commit 这类小状态机写成可检查模型。
6. **Rust kernel fuzzing**：外部输入边界必须主动寻找 panic-prone path，而不仅是语义错误返回。Asterinas 的 RusyFuzz 说明 `unwrap`、`expect`、unchecked index、overflow assertion 和 compiler-inserted assertion 是 Rust kernel 的独立风险面；Ousia 的 syscall、handle、IPC、VM range、allocation failure injection 和 object id/generation 输入都应进入 fuzz 计划。
