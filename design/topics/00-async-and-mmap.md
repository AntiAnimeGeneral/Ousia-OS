...
本文不再定义 Ousia 的完整通信模型。统一通信基座、Portal / Operation / Continuation / EventPort / SharedQueue 等原语归属 [08-communication-fabric.md](../core/08-communication-fabric.md)。本文只讨论一个边界问题：Ousia 坚持异步优先时，如何与 `mmap` 这种硬件层面的同步缺页模型共存。
...
内核提供统一的 EventPort / WaitSet：`wait(events, timeout)` / `signal` / `cancel`。Event 来源包括 Operation completion、TimerExpired、DeviceInterrupt、StreamReadable、MemoryObjectLost、QueueReadable、FenceReached、ProcessTerminated 等。完整事件来源和通信路径见 [08-communication-fabric.md](../core/08-communication-fabric.md)。
...
- [05-pager-and-memory.md](../core/05-pager-and-memory.md) — Pager 超时和崩溃模型
- [06-compute-and-scheduling.md](../core/06-compute-and-scheduling.md) — 缺页 stall 时的调度行为
- [08-communication-fabric.md](../core/08-communication-fabric.md) — 统一通信基座
