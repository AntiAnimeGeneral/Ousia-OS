...
本文是 Ousia OS 的需求库和抽象推导索引。它承接 [target.md](./target.md) 的愿景目标，把第一阶段必须满足的能力写成可追踪需求，并记录这些需求如何推出核心抽象。
...
| 内容 | 放置位置 | 说明 |
| ---- | -------- | ---- |
| 愿景目标 | [target.md](./target.md) | 描述系统方向和默认秩序，保持短而稳定。 |
| 硬需求 | 本文 | 使用 `R#` 编号，作为第一阶段验收入口。 |
| 抽象推导 | 本文 | 使用 `D#` 编号，记录哪些需求组合推出哪些抽象。 |
| 稳定抽象契约 | [core/](./core/) | 一旦推导结论被接受，抽象定义、边界和协议写入 owning core 文档。 |
| 论证细节 | [deep-dives/](./deep-dives/) 或 [reference/](./reference/) | 保存候选方案、性能分析、调研材料和开放问题。 |

抽象推导结论的落点按 ownership 决定：通信原语落到 [02-communication-fabric.md](./core/02-communication-fabric.md)，MemoryObject 和 Pager 落到 [03-pager-and-memory.md](./core/03-pager-and-memory.md)，驱动/IO 原语落到 [04-driver-and-kernel.md](./core/04-driver-and-kernel.md)，Object Namespace / FS Provider / Object Store 落到 [07-data-and-filesystem.md](./core/07-data-and-filesystem.md)，Package Cell 落到 [08-package-cell.md](./core/08-package-cell.md)。
...
| D6 | R7 + R8 | 兼容域不能绕过原生权限模型；兼容域网关必须把 POSIX 资源翻译为受 Capability 约束的原生对象。 | [01-compatibility.md](./topics/01-compatibility.md), [01-capsule-and-capability.md](./core/01-capsule-and-capability.md) |
