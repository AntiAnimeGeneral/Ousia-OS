...
| R7   | 权限必须可组合、可审计、可失效         | 打开、挂载、mmap、DMA、跨 Capsule 传递都必须由 Capability 表达；直接句柄、短租约和硬件映射必须可撤销或失效，未登记转发不承诺完整级联撤销 | Capability、ObjectHandle、DMA capability   |
...
| D6   | R7 + R8       | 兼容域不能绕过原生权限模型；兼容域网关必须把 POSIX 资源翻译为受 Capability 约束的原生对象，并按能力类别处理撤销、租约或失效通知。                    | [01-compatibility.md](./topics/01-compatibility.md), [01-capsule-and-capability.md](./core/01-capsule-and-capability.md)                 |
