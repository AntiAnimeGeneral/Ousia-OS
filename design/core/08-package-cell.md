...
> 承接 [target.md](../target.md) 中的软件交付、依赖治理与运行环境目标。

## 讨论范围

Package Cell 是 Ousia OS 最核心的抽象之一。本文讨论它的结构设计、依赖解析策略、生命周期管理，以及它和传统包管理器的本质区别。
...
cell:
  ...
  publisher: "did:Ousia OS:pubkey:..." # 发布者身份
  ...
  runtime:
    base: "Ousia OS-base-24.04" # 基础运行环境
    env:
      - name: "LOG_LEVEL"
        value: "info"
...
传统系统的环境变量是全局状态的典型。Ousia OS 禁止 Cell 声明依赖这些。运行时环境的生成逻辑：
...
Ousia OS 第一阶段不追求“全局唯一版本”的通用约束求解，而是通过限制约束语言、允许不兼容版本并存、固定选择规则来保证解析可复现。算法概要：
...
确定性的关键是：**相同输入 → 相同输出**。复杂冲突不通过全局搜索强行合并；无法安全合并时，优先把版本隔离在各自 Capsule 的依赖视图中。

多版本并存只在隔离的 Capsule、私有依赖视图或明确版本化的服务协议边界内成立。共享同一地址空间的插件、要求进程内单例的运行时、内核/驱动 SDK ABI、以及未版本化的服务协议不能简单并存；这些场景必须由宿主或 Service Graph 选择一个兼容版本，或返回解析错误。
...
| 维度       | apt/dpkg            | Docker         | Nix/Guix      | Ousia OS Package Cell |
| ---------- | ------------------- | -------------- | ------------- | --------------------- |
| 依赖解析   | 全局约束 + 人工仲裁 | 无（全量快照） | 声明式/确定性 | 受限约束 + 确定性     |
| 多版本并存 | 否                  | 是（全量）     | 是            | 是（原生）            |
...
Ousia OS 从 Nix/Guix 借鉴了内容寻址和确定性解析；从 Docker 借鉴了环境封装思想；但把这些从附加工具提升为系统原生能力。

---

## 开放问题

1. **base 环境的版本管理**：`Ousia OS-base-24.04` 的更新如何不破坏已有 Cell？是否需要类似 NixOS channel/profile generation 的模型？
2. **编译依赖 vs 运行依赖**：目前声明中没有区分。是否需要 `build-deps` 和 `runtime-deps`？
3. **GC 策略**：如何判断一个 Cell "不再被需要"？纯引用计数 vs 基于时间的保留策略？
4. **Cell 签名验证的信任根**：发布者公钥如何分发和验证？这需要 [05-identity-and-accounts.md](../topics/05-identity-and-accounts.md) 的身份与信任模型。

---

## 相关章节

- [01-capsule-and-capability.md](./01-capsule-and-capability.md) — Capsule 如何消费 Cell 的能力声明
- [06-service-graph.md](./06-service-graph.md) — 服务发现与版本协商
- [05-identity-and-accounts.md](../topics/05-identity-and-accounts.md) — 发布者身份与签名验证
- [06-roadmap.md](../topics/06-roadmap.md) — Package Cell 原型的落地顺序
