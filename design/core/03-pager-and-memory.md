# 03 — MemoryObject 与 Pager 边界

> 承接 [target.md](../target.md) 中的 MemoryObject 与 VM 协作目标。

## 讨论范围

MemoryObject 是 Ousia OS 的可分页内存对象抽象。它同时服务两种 FS 放置候选：纯用户态 FS 方案中，MemoryObject 由用户态 Pager 供页；纯内核态 FS 方案中，MemoryObject 可由内核 Object Store / page cache 直接供页。本文讨论 VM 机制、Pager 协议候选、崩溃模型，以及与传统 `mmap` 的对比。
...
Ousia 的 FS 放置仍在纯用户态 FS 与纯内核态 FS 之间裁决。二者对 `mmap` 的处理不同：

- 纯用户态 FS：缺页处理仍由内核驱动，但页面内容来自用户态 Pager。Pager 通信必须是 VM fast path，而不是普通业务 RPC。
- 纯内核态 FS：缺页后内核直接进入 Object Store / page cache，页面内容和 dirty/writeback 状态在内核内闭环。

MemoryObject 的核心要求是不把 `mmap(file)` 固化为 POSIX 语义。路径、标签、版本和权限选择先解析为 ObjectHandle / MemoryObject Capability；内核 VM 只处理 MemoryObject、offset、fault type、映射权限和失效状态。
...
```
1. 创建
   FS 权威方创建 MemoryObject
   → 纯用户态 FS：用户态存储服务调用 Kernel::create_memory_object(size, flags) 并绑定 Pager 通道
   → 纯内核态 FS：内核 Object Store 直接创建 MemoryObject 并绑定内部 page cache
...
3. 缺页处理
   Capsule 访问未填充页面
   → 纯用户态 FS：内核发送 PageRequest{mo_id, offset, type: READ|WRITE} 到 Pager
   → 纯内核态 FS：内核 Object Store / page cache 直接提供页面
   → 内核填充页表
   → Capsule 继续执行

4. 回写
   Capsule 修改了页面（dirty）
   → 纯用户态 FS：内核在内存压力下或 Pager 请求时，发送 FlushRequest 给 Pager
   → 纯内核态 FS：内核 writeback 路径直接关联 Object Store 事务
   → 持久化确认后，内核可以回收该页框
...
纯用户态 FS 不直接把 POSIX FUSE 回调接入 VM。它通过 FS Provider 接口把 ObjectHandle / Version / Lease 转换为可分页 MemoryObject；Pager 再以 page fault 粒度供页、回写和失效。远程 FS 也是同一路径：远端对象先成为本地 Remote-backed MemoryObject，缺页命中本地 cache 或触发 Provider fetch，完成后由 Pager 向内核交付页面。
...
## Pager / Object Store 崩溃模型

本节主要约束纯用户态 FS 方案。纯内核态 FS 方案下，Object Store 崩溃等价于内核故障，恢复重点转为 journal replay、panic path 和离线修复。
...
| 维度       | 传统 mmap (Linux)  | 纯用户态 FS + Pager       | 纯内核态 FS / Object Store |
| ---------- | ------------------ | ------------------------- | -------------------------- |
| 缺页处理者 | 内核 (page cache)  | 内核 + 用户态 Pager       | 内核 Object Store          |
| 页缓存管理 | 内核               | 内核页框 + Pager 策略     | 内核 page cache            |
| 回写       | 内核 flusher       | Pager / FS 主导           | 内核 writeback             |
| 崩溃恢复   | 内核 FS 保证一致性 | Pager / FS 事务日志保证   | 内核 journal 保证          |
| 零拷贝     | 是                 | 是，共享页池              | 是，内核 page cache        |
| 可定制性   | 低                 | 高                        | 中，取决于内核 ABI         |
| 故障模式   | 内核 panic         | Pager 崩溃 → let it crash | 内核故障                   |
...
内核维护页框的访问频率信息。纯用户态 FS 方案中，Pager 可以提供额外的冷热分层提示（如"这个页面属于冷数据"）；纯内核态 FS 方案中，这些策略由内核 Object Store / page cache 实现。
...
4. **FS 放置裁决**：MemoryObject 的主要供页方应是用户态 Pager，还是内核 Object Store / page cache？

---

## 相关章节

- [07-data-and-filesystem.md](./07-data-and-filesystem.md) — Object Store 与 FS 放置候选
