...
seL4 的优雅之处在于边界极窄：内核只管理少量对象和 capability，复制、派生和删除都由内核记录；`CNode_Revoke` 删除某个 capability 的所有子 capability。它强的是**内核可见能力派生关系上的硬撤销**，不是服务状态回滚、缓存数据追回或业务语义撤销。

Zircon 的启发在于把 capability 风格做成高级 handle/object API：用户态持有 typed handle，内核对象、rights、generation 和生命周期由内核检查，Channel、VMO、Process、Thread 和 Driver Framework 都能通过统一 handle 语义暴露。

Ousia OS 不应宣称自己在能力安全上天然比 seL4 更强，也不应把 seL4 的 CSpace/Untyped/retype plumbing 直接变成用户库负担。第一阶段应建立 Ousia 原生高级 capability kernel：handle/object、channel/call、MemoryObject/VM、VFS/Object Namespace、Process/Thread 和 Driver resource handle 是主线 API；seL4 提供硬撤销和失败纪律参考，Zircon 提供高级 API 和对象模型参考。
...
1. **硬能力层**：第一阶段以内核可见 `Handle`、kernel object、rights、generation、object lifetime 和 handle transfer 为核心，维护可审计的派生/转交关系。Portal、Continuation、EventPort、MemoryObject、IOBuffer、IOQueue、Device Resource、IOMMU mapping、ObjectHandle 等可以是内核一等对象或明确的服务对象，但都必须通过同一能力边界表达授权、撤销和失效。内核提供 `delete(handle)`、`revoke_descendants(handle)`、`destroy_object(object)` 和 `invalidate_generation(object)` 等语义。撤销后，后代句柄、映射、等待者和 fast-path descriptor 必须进入明确错误状态。
...
第一阶段不需要实现所有服务语义撤销，但第 0 层内核能力合同必须先冻结 Ousia handle/object 语义：

- `GRANT` 或等价权限控制派生和转发；没有该权限时只能使用，不能复制或降权派生
- 派生 capability 的 rights 必须是父 capability rights 的子集
- `delete(handle)` 删除当前 handle table entry 或等价持有点中的句柄
