...
Ousia OS 不应宣称自己在微内核能力核心上天然比 seL4 更强。第一阶段应先在 Rust 中对齐 seL4 baseline 的能力核心、CSpace/CNode、Untyped/retype、delete/revoke、IPC、Reply、Notification 和 TCB 语义；在它之上，Ousia 再提供更面向现代系统的类型化资源、Service Graph、租约、对象 generation、IOBuffer/IOMMU 和 Package Cell 语义。换句话说，Ousia 的强大不来自替代 seL4 的能力核心，而来自把这个硬核心扩展成完整平台语义。
...
1. **硬能力层**：第一阶段先以 seL4 baseline 的 CSpace/CNode、Untyped、Endpoint、Notification、Reply、TCB、Frame 和 address-space 相关对象为内核可见对象，维护 capability derivation tree。Portal、Continuation、EventPort、MemoryObject、IOBuffer、IOQueue、Device Resource、IOMMU mapping 等是后续 Ousia 扩展或映射对象，进入内核前必须先证明不会替代或削弱 baseline。内核提供 `delete(handle)`、`revoke_descendants(handle)`、`destroy_object(object)` 和 `invalidate_generation(object)` 等语义。撤销后，后代句柄、映射、等待者和 fast-path descriptor 必须进入明确错误状态。
...
第一阶段不需要实现所有服务语义撤销，也不把 Ousia Portal/Operation/Communication Fabric 当作 kernel baseline 对象；第 0 层内核能力合同必须先对齐 seL4 baseline：
